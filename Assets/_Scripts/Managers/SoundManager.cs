...
namespace My.Scripts.Managers
{
    public class SoundManager : PersistentSingleton<SoundManager>
    {
        ...
        private const float SIDE_THRUSTER_VOLUME = 0.8f;   // √ромкость бокового двигател€
        private const float UP_THRUSTER_VOLUME = 0.6f;     // √ромкость верхнего двигател€
        private const float MAX_COMBINED_VOLUME = 1f;      // ћаксимальна€ комбинированна€ громкость
        ...
        private void UpdateThrusterAudio()
        {
            ...
            // ¬ычисл€ем целевую громкость и панораму
            ...
            // ѕлавно переходим к новым значени€м
            StopThrusterFadeCoroutine();
        ...
        private void CalculateThrusterParameters(out float volume, out float pan)
        {
            ...
            // Ћевый двигатель: звук смещаетс€ ¬ѕ–ј¬ќ (положительный pan)
            ...
            // ѕравый двигатель: звук смещаетс€ ¬Ћ≈¬ќ (отрицательный pan)
        ...
        {
            // ∆дЄм конца кадра, чтобы все событи€ Force успели сработать
        ...
        {
            // —брасываем состо€ние перед новым кадром
            _isLeftThrusterActive = false;
            _isRightThrusterActive = false;
            _isUpThrusterActive = false;

            // «апускаем отложенную проверку в конце кадра
